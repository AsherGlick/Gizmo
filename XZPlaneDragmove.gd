...
func _input(event:InputEvent):
	if selected:
		if event is InputEventMouseButton:
			...
			var angle_diff = (camera.translation - (self.start_position + delta_offset)).dot(camera.transform.basis.z)
			if (angle_diff < 0):
				print(angle_diff)
				delta_offset = -delta_offset + (self.start_position - camera.translation)
